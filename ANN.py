'''
    1. 인공 신경망
        ...
        5) 경사 하강법
            - 신경망 훈련의 목표는 최적 파라미터를 찾는 것임
                - 파라미터는 가중치와 편향
            - 최적 파라미터 손실 함수가 최솟값일 때의 파라미터를 뜻함.
            ...
            - 손실 함수(loss function)
                ...
                - 손실 함수값이 최소가 되는 가중치와 편향을 구하는 작업(신경망 훈련)
            ...
            - 경사 하강법 (gradient descent)
                ...
                - 절차
                    ...
                    - 현재 위치의 기울기가 0이 될 때까지 1 ~ 2단계를 반복함
                        - 손실 함수가 최소가 될 때까지임
                - 학습률(learning rate)
                    - 기울기 방향으로 얼마만큼 이동할 지 결정하는 값
                    - 가중치를 한 번에 얼마나 갱실할 지 결정
                    - 다음 가중치
                        - 기존 가중치에서 '학습률과 기울기를 곱한 값'을 뺀 값
                    - 학습률에 따라 훈련 속도가 달라짐
                        - 한 걸음에 얼마만큼 이동할지 결정하는게 학습률임
                            - 학습률이 너무 크면 훈련 속도는 빠르지만, 더 빠른 길로 이어지는 지점을 건너뛰어서 최적 가중치를 찾지 못할 수도 있음.
                            - 학습률이 너무 작으면 훈련 속도가 느려짐
                        - 하이퍼 파라미터이기에 우리가 직접 설정해줘야 함
                            - 보통 0.1 ~ 0.0001 범위의 값을 사용
                - mini batch
        ...
        6) 순전파 & 역전파
            ...
            - 절차
                ...
                - 기울기를 바탕으로 가중치를 갱신함. 이 때 출력층 => 입력층 방향으로 가중치를 갱신(역전파)
    ...
    2. 딥러닝 알고리즘 성능 향상
        1) 드롭아웃(dropout)
            - 과대적합을 방지하기 위하여 신경망 훈련 과정에서 무작위로 일부 뉴련을 제외하는 기법
