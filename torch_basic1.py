'''
    1. 파이토치(PyTorch) 개요
        ...
        2) GPU에서 텐서 조작 및 동적 신경망 구축이 가능한 프레임워크
            ...
            - 병렬연산에서 GPU의 속도는 CPU의 속도보다 훨씬 빠르므로 딥 러닝 학습에서 GPU 사용은 필수임.
    ...
    3. 벡터
        ...
        텐서
            1) 3차원 이상의 배열 형태
                ...
                - 3차원의 축(채널) : axis=2(텐서)
            2) torch.tensor()
            3) 텐서 간의 연산을 수행할 때, 기본적으로 두 텐서가 같은 장치에 있어야 함

    4. 파이토치 특징
        1) 신경망은 연산 그래프를 이용하여 계산을 수행
        2) 네트워크이 학습 될 때 손실 함수의 기울기가 가중치와 바이어스(절편)를 기반으로 계산되며 이후 경사 하강법을 사용하여 가중치가 업데이트
    ...
    5. 파이토치 아키텍처
        1) 파이토치 API 계층 : 사용지의 편의성 여러 패키지들 제공
            - torch : GPU를 지원하는 텐서 패키지. CPU 뿐만 아니라 GPU에서 연산이 가능함(빠른 속도로 많은 양의 계산이 가능)
                        파이토치엫서는 텐서가 핵심
            - torch.autograd : 자동 미분 패키지
            - torch.nn : 신경망 구축 및 훈련 패키지
        2) 파이토치 엔진(라이브러리)
            - C++로 감싼(Wrapping) 다음 Python API 형태로 제공
