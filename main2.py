...
dataset = pd.read_csv('resources/car_evaluation.csv')
print(dataset.head())
...
outputs = pd.get_dummies(dataset.output)  # 가변수(dummy variable)로 만들어주는 함수 (0, 1)
...
class Model(nn.Module):  # 클래스 형태로 구현되는 모델은 nn.Module을 상속받음
    '''
        self : 첫번째 파라미터로 self로 지정해야 함, 자기 자신을 가리킴
    ...
    def forward(self, x_categorical):                      # 학습 데이터를 입력받아 연산을 진행
...
'''
    ...
    은닉층의 뉴런을 정의는 여러 크기로 지정하여 테스트 해보는 것이 학습에 도움이 될 것임.
...
print(f"loss : {loss:.8f}")                     # 테스트 데이터 셋에 대한 손실값을 출력
                                                # 훈련 데이터 셋에서 도출된 값과 비슷하므로 과적합은 발생하지 않음
...
# 신경망에서 필요한 모든 파라미터를 무작위로 선택했다는 점을 감안할 때, 약 74%의 정확도는 나쁘지 않음
